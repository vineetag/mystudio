 ...
 *
 * The userSecret is issued exactly once at registration, so it is persisted
 * in pt_snaptrade_users — a table with RLS enabled and NO policies, readable
 * only through the service role. It never reaches the client. (It is a
 * SnapTrade API secret, not a brokerage credential — those never leave
 * SnapTrade; see the PRODUCT_SPEC guardrails.)
 */
