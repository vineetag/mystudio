...
export function StoryCard({ story }: { story: StoryCardData }) {
  ...
  return (
    ...
    >
      <div
        className={`mb-4 flex h-24 items-center justify-center rounded-card text-5xl bg-theme-${story.theme}-bg`}
        aria-hidden="true"
      >
        {story.illustration ?? "📖"}
      </div>
      <span
        className={`text-xs font-semibold uppercase tracking-wide text-theme-${story.theme}-text`}
      >
        {themeLabel}
      </span>
