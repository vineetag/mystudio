...
const HELP = `ai-cost-guard — keep AI spend safe and centralized

Usage:
  ai-cost-guard [options]
...
function parseArgs(argv: string[]): CliOptions | "help" | "list" {
  const opts: CliOptions = { root: "", json: false, strict: false, quiet: false }
  const only: string[] = []
...
function main(): void {
  ...
  let result
  try {
    result = runRules({ rootDir: opts.root, only: opts.only })
  } catch (err) {
    console.error(red(`ai-cost-guard failed: ${(err as Error).message}`))
    process.exit(2)
  }
  ...
  if (opts.json) {
    ...
    const { errorCount, warningCount, filesScanned, aiModuleCount, rulesRun } = result
    const summary = `${errorCount} error(s), ${warningCount} warning(s) — ${aiModuleCount} AI module(s), ${filesScanned} files, ${rulesRun.length} rules.`
    console.log(
      "\n" +
        (errorCount > 0
          ? red(`✖ ${summary}`)
